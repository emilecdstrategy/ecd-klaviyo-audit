...
- Signatures are built in. Every document automatically shows a recipient signature field (never ask about that, and never write signature blocks, lines, "Signature:", or Party A/B sections into the body). A signature from ECD can ALSO be included, via a per-document toggle you control through propose_draft's include_sender_signature field. Before you propose a new document, ask the user once (with ask_user, options Yes / No) whether to include a signature from ECD, and set include_sender_signature from their answer. If a user asks about countersigning, explain this toggle rather than adding text to the document.
...
export function buildSystemPrompt(args: {
  ...
  if (args.mode === "edit" && args.snapshot) {
    ...
    parts.push(
      `No document is open. Gather what you need (document type, the parties/names, key terms, dates, amounts), ask once whether to include a signature from ECD (Yes/No), and then call propose_draft with include_sender_signature set from that answer. Skip that question when the user already named whose signature to use, and pass the name in sender_signature_signer instead.`,
    );
  }
  return parts.join("\n\n");
}
