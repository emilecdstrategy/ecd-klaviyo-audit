/**
 * Browserless (https://browserless.io) headless-browser pass. Runs one /function
 * call that returns BOTH the above-the-fold viewport screenshot AND the real
 * bounding boxes of the page's elements, measured at the same render so the two
 * always line up. The web-audit pipeline then lets the AI pick an element id
 * instead of guessing pixel coordinates.
 *
 * Enabled only when BROWSERLESS_TOKEN is set; otherwise the pipeline falls back
 * to the ScreenshotOne image + model-estimated coordinates.
 */
...
// Puppeteer module run inside Browserless. Collects visible, above-the-fold
// elements, tightly clamped to the viewport, deduped, capped, then takes the
// viewport screenshot. Kept as a plain string so it ships to /function as-is.
const FUNCTION_CODE = `
export default async ({ page, context }) => {
  const { url, width, height } = context;
  await page.setViewport({ width, height, deviceScaleFactor: 1 });
  await page.goto(url, { waitUntil: "networkidle2", timeout: 45000 });
  // Best-effort: remove fixed cookie/consent overlays that cover the fold.
  await page.evaluate(() => {
    const sels = ['[id*="cookie" i]','[class*="cookie" i]','[id*="consent" i]','[class*="consent" i]','[aria-label*="cookie" i]','[class*="gdpr" i]'];
    for (const s of sels) {
      document.querySelectorAll(s).forEach((el) => {
        const cs = getComputedStyle(el);
        if (cs.position === "fixed" || Number(cs.zIndex) > 1000) el.remove();
      });
    }
  }).catch(() => {});
...
export async function captureViewportWithElements(input: {
  url: string;
  ...
  try {
    const res = await fetch(`${base}/function?token=${encodeURIComponent(token)}`, {
      ...
      body: JSON.stringify({ code: FUNCTION_CODE, context: { url: input.url, width: dim.width, height: dim.height } }),
      signal: ctrl.signal,
    });
    if (!res.ok) {
    ...
    const msg = e instanceof Error && (e.name === "AbortError" || /abort/i.test(e.message)) ? "browserless_timeout" : (e instanceof Error ? e.message : "browserless_failed");
