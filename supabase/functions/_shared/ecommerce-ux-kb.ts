...
export const GENERAL_LAYOUT_RULES = [
  ...
  "FITTING TO ONE ROW MEANS ONE ROW. When a fix says a navigation or button row should fit on a single line, the output must show exactly ONE row with nothing wrapping to a second line. Actually shorten or group the labels enough to make that true (fewer visible items, a single 'More' menu), rather than restyling and leaving the wrap in place. Count the rows before you finish.",
