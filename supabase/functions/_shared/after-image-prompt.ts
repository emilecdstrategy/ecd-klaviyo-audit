...
export function buildEditPrompt(
  ...
  const common = [
    ...
    `- NEVER RENDER LAYOUT VOCABULARY AS PAGE TEXT. Words used in these instructions to describe page furniture ("carousel", "hero", "grid", "thumbnail strip", "banner", "badge", "CTA", "sticky bar", "drawer") describe what to BUILD. They must never appear as visible words in the image. Watch the headings and the product title especially: appending a word like "Carousel" to a real product name corrupts the client's own copy and makes the whole image unusable. Product titles, prices and product names are reproduced from the source EXACTLY, character for character.`,
    `- KEEP EVERY THUMBNAIL. If the source shows a strip of thumbnail images under or beside the main photo, reproduce the SAME NUMBER of thumbnails. Count them in the source and count them in your output. Dropping thumbnails deletes real product photographs the client paid for, which fails this task even if the layout around them improved.`,
  ...
  const hardConstraints = [
    ...
    `8. A WIDE BANNER KEEPS ITS HEIGHT UNLESS A FIX ASKS OTHERWISE. By default a full-width hero or category banner stays exactly as tall as it is in the source. If, and only if, one of the fixes above explicitly asks to trim or shorten it, then do so THIS way: show a shorter band of the SAME photo at the SAME scale, keeping the subject the same size and in the same place, exactly as if a window over it got shorter. Never squash, stretch, or zoom the photo to fit a shorter band, and never re-centre the subject. Every other photo on the page still keeps its full framing.`,
  ].join("\n");
