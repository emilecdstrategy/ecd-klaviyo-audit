...
export default function ReportCover({
  ...
  brandSubtitle = 'Klaviyo Email Audit Report',
  deliverable = 'Klaviyo Lifecycle Audit',
  description = "A strategic review of your Klaviyo account. What's working, what needs attention, and where revenue is being left on the table.",
}: {
  companyName: string;
  preparedDate: string;
  websiteUrl?: string | null;
  identifiedOpportunity?: ReactNode;
  /** Defaults describe a Klaviyo audit; the web report passes its own so the two
   *  covers stay one component rather than drifting into two designs. */
    ...
    <div className="relative mb-10 overflow-hidden rounded-3xl text-white shadow-xl shadow-brand-primary/15 ring-1 ring-white/10">
      ...
      <div className="relative px-8 py-8 sm:px-12 sm:py-10 lg:px-14 lg:py-12">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between sm:gap-8">
          <ReportBrandMark size="lg" inverted subtitle={brandSubtitle} />
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm sm:shrink-0 sm:justify-end sm:text-right">
            ...
            <div>
              ...
              <p className="mt-0.5 font-medium text-white/90">{deliverable}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 max-w-3xl">
          ...
          <p className="mt-4 text-base leading-relaxed text-white/80 sm:text-lg max-w-2xl">{description}</p>
        </div>
